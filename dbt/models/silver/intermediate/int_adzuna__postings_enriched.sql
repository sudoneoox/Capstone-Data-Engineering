...
WITH base AS (
    ...
    SELECT
        ...
        latitude,
        longitude,
        {{ col("location.display_name") }} AS raw_location,
        {{ col("company.display_name") }} AS company,
...
geo AS (
    SELECT DISTINCT
        ...
        LOWER(TRIM(state_full)) AS state_norm,
        city,
        county,
        state_full AS state
...
SELECT
    CAST(id AS {{ dbt.type_string() }}) AS posting_id,
